...
def train_cc50_median_classification():
    RAW_TARGET_FOR_MEDIAN = config.TARGET_CC50
    TASK_NAME = "CC50_Median_Classification"
    ...
    if X_train is None or RAW_TARGET_FOR_MEDIAN not in Y_train.columns:
        print(f"Could not load data or raw target '{RAW_TARGET_FOR_MEDIAN}' missing. Aborting.")
        return None, {}

    y_train_raw_for_median = Y_train[RAW_TARGET_FOR_MEDIAN]
    y_test_raw_for_median = Y_test[RAW_TARGET_FOR_MEDIAN]

    train_median_value = y_train_raw_for_median.median()
    print(f"Median for {RAW_TARGET_FOR_MEDIAN} (on training data): {train_median_value:.4f}")

    y_train = (y_train_raw_for_median > train_median_value).astype(int)
    y_test = (y_test_raw_for_median > train_median_value).astype(int)

    print(f"Train class distribution ({RAW_TARGET_FOR_MEDIAN} > Median):\n{y_train.value_counts(normalize=True)}")
    ...
    models_params = {
        "LogisticRegression": (
            LogisticRegression(random_state=config.RANDOM_STATE, solver='liblinear', class_weight='balanced',
                               max_iter=1000),
            {'C': [0.1, 1, 10]}),
    ...
    for name, (model, params) in models_params.items():
        ...
        try:
            ...
            current_test_results = evaluate_classification(y_test, y_pred_test, y_proba_test, model_name=name,
                                                           task_name=TASK_NAME)
    ...
    print(f"\n\n--- Model Comparison for {TASK_NAME} (Sorted by Test ROC_AUC) ---")
    print(results_df)
    results_df.to_excel(config.REPORT_DIR_ML / f"{TASK_NAME.lower()}_model_comparison.xlsx")

    if not results_df.empty:
        best_model_name_on_test = results_df['ROC_AUC'].idxmax(skipna=True)
        if pd.notna(best_model_name_on_test):
            print(
                f"\nBest model for {TASK_NAME} based on Test ROC_AUC: {best_model_name_on_test} (ROC_AUC: {results_df.loc[best_model_name_on_test, 'ROC_AUC']:.4f})")
...
if __name__ == '__main__':
    train_cc50_median_classification()